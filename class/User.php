...
class User extends Dbconfig {	
	...
	public function getUserList(){		
		...
		while( $users = mysqli_fetch_assoc($result) ) {		
			...
			$userRows[] = $users['address'];			
			$userRows[] = $users['mobile'];	
			$userRows[] = $users['birth_date'];	
			$userRows[] = $users['hired_date'];
			$userRows[] = $status;						
	...
	public function updateUser() {
		if($_POST['userid']) {	
			$updateQuery = "UPDATE ".$this->userTable." 
			SET first_name = '".$_POST["firstname"]."', middle_name = '".$_POST["middlename"]."', last_name = '".$_POST["lastname"]."', address = '".$_POST["address"]."', birth_date = '".$_POST["birthdate"]."', hired_date = '".$_POST["hireddate"]."', email = '".$_POST["email"]."', gender = '".$_POST["gender"]."', martial_status = '".$_POST["martialstat"]."', mobile = '".$_POST["mobile"]."' , designation = '".$_POST["designation"]."', status = '".$_POST["status"]."', type = '".$_POST['user_type']."'
			WHERE id ='".$_POST["userid"]."'";
			$isUpdated = mysqli_query($this->dbConnect, $updateQuery);		
		}	
	}	
	...
	public function addUser () {
		if($_POST["email"]) {
			$authtoken = $this->getAuthtoken($_POST['email']);
			$insertQuery = "INSERT INTO ".$this->userTable."(first_name, middle_name, last_name, address, birth_date, hired_date, email, password, gender, martial_status, mobile, designation, type, status, authtoken) 
				VALUES ('".$_POST["firstname"]."', '".$_POST["middlename"]."', '".$_POST["lastname"]."', '".$_POST["address"]."', '".$_POST["birthdate"]."', '".$_POST["hireddate"]."', '".$_POST["email"]."', '".md5($_POST["password"])."', '".$_POST["gender"]."', '".$_POST["martialstat"]."', '".$_POST["mobile"]."', '".$_POST["designation"]."', '".$_POST['user_type']."', 'active', '".$authtoken."')";
			$userSaved = mysqli_query($this->dbConnect, $insertQuery);
		}
	}
